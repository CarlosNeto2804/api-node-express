...
class UserController{
    ...
    static async create(req, res){
        if(!req.body) return res.status(400).send("Formato de requisição inválido");
        try {
    ...
    static async findById(req, res){
        try{
            if(!req.params.id) return res.status(400).send("É necessário fornecer um Id")
    ...
    static async update(req, res) {
        if (!req.body._id) return res.status(400).send(
            "É preciso fornecer o ID do usuario para alterá-lo");
        try {
    ...
    static async remove(req, res) {
        if (!req.body._id) return res.status(400).send(
            "É preciso fornecer o ID do usuario para remove-lo");
        try {
